...
const jollofRice: RecipeType = {
  nom: "Jollof Rice",
  description: "Jollof, or jollof rice, is a rice dish from West Africa. The dish is typically made with long-grain rice, tomatoes, chilies, onions, spices, and sometimes other vegetables and/or meat in a single pot",
  imageURL:"https://www.preciouscore.com/wp-content/uploads/2021/12/Jollof-Rice-With-Chicken.jpg",
  ingredients: [rice, tomato, onion, chicken, pepper, oil],
  etapes: stepsJollofRice,

  ingredientsWithQuantity: [ 
    { ingredient: rice, quantity: 2, unit: "cups" },
    { ingredient: tomatoPaste, quantity: 1, unit: "can" },
    { ingredient: onion, quantity: 1 },
    { ingredient: redPepper, quantity: 1 },
    { ingredient: spices, quantity: 1, unit: "tablespoon" },
    { ingredient: rice, quantity: 1, unit: "cups" },
...
export const africanRecipes: RecipeType[] = allRecipes.filter((recette) => {
  const tagIds: string[] = recette.tags.map((tag) => tag.id);
  return tagIds.includes("african");
});

export const africanVegetarianRecipes: RecipeType[] = allRecipes.filter((recette) => {
  const tagIds: string[] = recette.tags.map((tag) => tag.id);
